...
<b>Background</b>: the Bitcoin white paper was released under the pseudo-name Satoshi Nakamoto on 2008-10-31, poignantly this was a time when the traditional banking system was in turmoil at the onset of a global financial crisis. The exact identity of Satoshi Nakamoto is not known, some speculate that Satoshi is actually a group rather than an individual. Satoshi circulated the paper on the Cypherpunks mailing list. Cypherpunks are a group of activists that advocate the use of cryptography and privacy enhancing technologies as a route to social and political change. Creating a digital money independent of the traditional banking system for use on the internet had been a long term objective of the libertarian minded Cypherpunks. Ironically much of the cryptography that led to the Cypherpunk movement was mostly developed in the 1970s within GCHQ (UK) and the NSA (USA). 

<b>Properties</b>: some of the main properties of the Bitcoin network  are:
...
Don't worry if this doesn't make sense to you at this stage, I recommend getting familiar with the following cryptographic and networking concepts, and then re-reading the abstract at the end of this post. Note proof-of-work is not introduced here since since it is discussed at length later in the post.

<p id="fn1"><b>Peer-to-peer</b>: a fundamental design feature of Bitcoin is that it is built as a peer-to-peer (P2P) network, as opposed to a client-server network. In a client-server architecture communication is usually to and from a central server providing resources. A P2P network is a distributed architecture, in which nodes are both conumers and suppliers of resources, removing the need for a centralised authority. In the case of Bitcoin a centralised authority would be provided by a financial institution. The use of P2P networks first became widely popular with music sharing applications such as Napster. 
...
<p id="fn5"><b>Best effort basis</b>: Best effort basis is an agreement that something will be attempted without any guarantee provided that it will succeed. Best-effort delivery describes a network service in which the network does not provide any guarantee that data is delivered or that delivery meets any quality of service.
</p>
...
The proof-of-work system is a puzzle that miners on the network compete to solve first. They do this by incrementing a nonce value in the block until a hash is achieved with sufficient number of leading zeros. So for example the first hash ever found was the following, note the previous block is all zeros since there was no previous block!
...
Again by block 500000 the difficult has increased again.
...
When a miner finds a hash with enough leading zeros, you can think of it as them winning that competition. They can then combine transactions into a block according to the rules of the network, and propagate that block on the network such that other miners can validate it.

Obtaining a distributed consensus is a major objective of Bitcoin, in order to achieve this the system has to overcome the Byzantine General's Problem, a classic problem faced by any distributed computer system network. The Byzantine General's problem is explained through a scenario in which multiple divisions of an army are laying seige to a city, against fierce resistance. Each division has a general, that can communicate to other generals via a messenger. A majority have to agree to attack or to defend, but the difficulty arises since some of the generals or messengers may be untrustworthy.
...
The total number of Bitcoins that can be mined is slightly less than 21 million, with the number of Bitcoins mined reducing over time, in a halving event approximately every 4 years, from 50 BTC per block, to 25 BTC, to 12.5 BTC etc. This leads to a deflationary currency, since the rate of increase in supply is reducing over time and will reach a stable number of coins around the year 2140. At this point the network will sustain on transaction fees alone and no new Bitcoins will be mined.

#### Further reading:
* <a href="https://en.bitcoin.it/wiki/Controlled_supply" target="_blank">Bitcoin supply</a>
...
A Merkle tree is a hash-based data structure that is a generalisation of the hash list. It is a tree structure in which each leaf node is a hash of a block of data, and each non-leaf node is a hash of its children. Typically, Merkle trees have a branching factor of 2, meaning that each node has up to 2 children.
